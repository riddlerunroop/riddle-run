...
const ADMIN_PASSWORD = "admin123"; // ← CHANGE BEFORE LAUNCH
...
export default function App() {
  ...
  const handleUseHint = () => {
    if (hintsAvailable > 0) { setHintsAvailable(p=>p-1); setShowHint(true); }
    else { setShowHint(true); } // In production: charge via Razorpay
    setHintsUsed(p=>p+1);
  };
            ...
            {isWaiting && isLevelCompleted ? (
              ...
              <>
                ...
                <div className="hint-row">
                  {!showHint
                    ? <button className="hint-btn" onClick={handleUseHint}>🔍 Hint {hintsAvailable>0?`(${hintsAvailable} free)`:"(₹29)"}</button>
